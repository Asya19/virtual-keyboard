...
window.onload = () => {
  ...
  function CreateKeyboard() {
    languageCheck();
    const CONTENT = `<textarea class="textArea"></textarea>
    ...
    for (const i in classForLengthBnt) {
      const addClass = classForLengthBnt[i];

      // eslint-disable-next-line camelcase
      // eslint-disable-next-line no-shadow
      const CONTENT = `<div class="oneBtn key ${addClass}" data-code="${keyCodes[i]}" data-ru-shift="${ru_shift[i]}" data-en-shift="${en_shift[i]}" data-ru="${ru[i]}" data-en="${en[i]}">
      ${getLayOutKeyboard(i)}</div>`;
      KEYBOARD.insertAdjacentHTML('beforeend', CONTENT);
    }

    const mes = '<p>Клавиатура создана в операционной системе Windows <br> Для переключения языка комбинация: левыe Shift + Alt<p>';
  ...
  function keyPress(event, button, dataCode) {
    ...
    if (dataCode === 'ArrowUp') {
      ...
      if (textBeforeCursor.length === 1 || textBeforeCursor[textBeforeCursor.length - 1].length >= 57) cursor = -57;

      // eslint-disable-next-line max-len
      else if (textBeforeCursor[textBeforeCursor.length - 1].length <= textBeforeCursor[textBeforeCursor.length - 2].length % 57) cursor -= (textBeforeCursor[textBeforeCursor.length - 2].length % 57) + 1;
      else cursor -= textBeforeCursor[textBeforeCursor.length - 1].length + 1;
    ...
    if (dataCode === 'ArrowDown') {
      ...
      // eslint-disable-next-line max-len
      else if ((textBeforeCursor[textBeforeCursor.length - 1].length % 57) > textAfterCursor[1].length) cursor += textAfterCursor[0].length + textAfterCursor[1].length + 1;

      // eslint-disable-next-line brace-style
      else if ((((textBeforeCursor[textBeforeCursor.length - 1].length) + textAfterCursor[0].length) > 57)) { cursor += textAfterCursor[0].length; }

      else { cursor += (textBeforeCursor[textBeforeCursor.length - 1].length % 57) + textAfterCursor[0].length + 1; }
    ...
    if (event.shiftKey || shift) {
      ...
      document.querySelectorAll('.key').forEach((e) => {
        if (e.dataset[lang] && !(e.classList.contains('specialBtn'))) {
          if (caps === 'on' && !(e.shiftKey || shift) && !(e.classList.contains('specialBtn'))) { e.innerHTML = e.dataset[lang].toUpperCase(); } else e.innerHTML = e.dataset[lang];
        }
      });
    }
  }
